...
export const getNetflitOriginals = () => async dispatch => {
    try{
      const  result = await axios.get(
        `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_networks=213`
      );
      dispatch({type: Types.GET_NETFLIT_ORIGINAL, payload: result.data.results});
    }catch (error) {
      console.log('Get Netflit API error: ', error);
    }
}
