  ...
  return (
    <MoviesDetailModal>
      ...
      <div className={`modal ${showModal ? 'showModal' : 'hideModal'}`}
        style={ movie ? {backgroundImage: `url(https://image.tmdb.org/t/p/original/${movie.backdrop_path || movie.posters_path})`, 
        ...
        <div className="container">
          <div className="movieInfo">
            ...
            <p className="statistical">
              <span className="rating">Rating Of Films: {movie && movie.vote_average * 10}</span>
              <span className="popurality">Popularity Of Films: {movie && movie.popularity}</span>
            </p>
            <p className="releaseDate">Release Date Of Films: {movie && 
            (moment(movie.release_date).format('DD-MM-YYYY') || 
            moment(movie.first_air_date).format('DD-MM-YYYY'))}</p>
            <p className="runtime">Runtime In The Movie: {movie && (movie.runtime || movie.episode_run_time)}</p>
